...
def main():
    ...
    # token cooc window used inside each segment (for event-local J_cc)
    ...
    ap.add_argument("--gamma_ctx", type=float, default=0.6)   # strength of context term (z_seg)
    ...
    print("=" * 80)
    print("HNET — EVENT-LOCAL TEMPLATES (psi per segment) + COO k-SPARSE ATTENTION + HIERARCHY")
    print("=" * 80)
    ...
    # ------------------------------------------------------------
    # Template prototypes Zc (global, but used to form psi locally)
    # ------------------------------------------------------------
    ...
    W_psi = Zc_norm @ Zc_norm.T  # compatibility prior

    # Precompute "template peers" globally for reporting:
    # top vocab words by dot(Xw, Zc[c])
    ...
    # ------------------------------------------------------------
    # Precompute global norm_out / norm_in for directional weights
    # (used inside segment to weight token->token cooc edges)
    # ------------------------------------------------------------
    # compute from entire token stream using same window
    ...
    norm_out_j = jnp.array(norm_out.astype(np.float32))
    norm_in_j = jnp.array(norm_in.astype(np.float32))
    ...
    # static k for top-k attention
    Kk = min(int(args.topk_attn), int(args.seg_len) - 1)
    NEG_LARGE = -1e9

    W = int(args.cooc_window)  # token cooc window inside each segment

    @jax.jit
    def segment_eventlocal_attention(word_ids_seg, noise_key):
        """
        Event-local templates psi, event-local directional J_pmi_seg,
        then k-sparse attention edges (COO).

        Returns:
          src, dst, w_attn  (S*Kk,)
          base_logits, tpl_bias, dir_bias  (S,S) for diagnostics
          psi (S,C)
          pair_mass (C,C) from attention edges
        """
        S = word_ids_seg.shape[0]

        Ew = Xw[word_ids_seg]  # (S,D)

        # segment embedding z_seg (mean)
        z_seg = jnp.mean(Ew, axis=0)  # (D,)

        # scores: e_t^T Z + gamma * z_seg^T Z  (broadcast context)
        token_scores = Ew @ Zc.T                        # (S,C)
        ctx_scores = (z_seg @ Zc.T)[None, :]            # (1,C)
        scores = token_scores + args.gamma_ctx * ctx_scores

        psi = jax.nn.softmax(scores / args.tau_psi, axis=1)  # (S,C)

        # token embedding anchored with psi@Zc + noise
        ...
        # --------------------------------------------
        # Event-local directional coupling J_pmi_seg
        # Build directed token edges within window W (fixed shape)
        # --------------------------------------------
        # Build src_pos, dst_pos for offsets 1..W
        src_list = []
        dst_list = []
        for d in range(1, W + 1):
            src_pos = jnp.arange(0, S - d)
            dst_pos = jnp.arange(d, S)
            src_list.append(src_pos)
            dst_list.append(dst_pos)
        src_pos = jnp.concatenate(src_list, axis=0)  # (Eseg,)
        dst_pos = jnp.concatenate(dst_list, axis=0)  # (Eseg,)

        w_src = word_ids_seg[src_pos]
        w_dst = word_ids_seg[dst_pos]

        # weight = 1/sqrt(norm_out[w_src] * norm_in[w_dst])
        denom = jnp.sqrt(norm_out_j[w_src] * norm_in_j[w_dst] + 1e-8)
        w_edge = 1.0 / denom  # (Eseg,)

        psi_src = psi[src_pos]                 # (Eseg,C)
        psi_dst = psi[dst_pos]                 # (Eseg,C)

        # J_mass_seg = (psi_src*w)^T @ psi_dst
        J_mass_seg = (psi_src * w_edge[:, None]).T @ psi_dst  # (C,C)
        J_row_seg = J_mass_seg / (jnp.sum(J_mass_seg, axis=1, keepdims=True) + 1e-12)

        # PMI-like contrast within segment
        pi_seg = jnp.mean(psi, axis=0)  # (C,)
        base_seg = pi_seg[:, None] * pi_seg[None, :]
        J_pmi_seg = jnp.log((J_row_seg + 1e-12) / (base_seg + 1e-12))
        J_pmi_seg = J_pmi_seg - jnp.mean(J_pmi_seg)
        J_pmi_seg = J_pmi_seg / (jnp.std(J_pmi_seg) + 1e-6)

        # directional bias (soft)
        dir_bias = (psi @ J_pmi_seg) @ psi.T

        # --------------------------------------------
        # Add salience + anti-hub + mask and compute sparse attention
        # --------------------------------------------
        ...
        # mask far and self with NEG_LARGE
        logits = jnp.where(local_attn, logits, NEG_LARGE)
        logits = logits - 1e9 * jnp.eye(S, dtype=logits.dtype)

        # top-k per row (static Kk)
        ...
        w_attn = probs.reshape(-1)            # (S*Kk,)

        # template flow mass from sparse edges: (psi[src]*w)^T @ psi[dst]
    ...
    print("\nCompiling event-local COO attention (one-time JAX cost)...")
    dummy = tok_ids[starts[0]:starts[0] + args.seg_len].copy()
    dummy[dummy < 0] = 0
    _ = segment_eventlocal_attention(jnp.array(dummy, dtype=jnp.int32), key)[0].block_until_ready()
    print("Compilation finished.\n")

    # ------------------------------------------------------------
    # Run segments, collect edges and template flows
    # ------------------------------------------------------------
    all_edges = []
    ...
    for seg_i, start in enumerate(starts, 1):
        ...
        src, dst, w_attn, baseL, tplB, dirB, psi, pair_mass = segment_eventlocal_attention(seg_j, nk)
        ...
        for s_idx, d_idx, ww in zip(src_np, dst_np, w_np):
            ...
            all_edges.append({
                ...
                "wid_i": wi,
                "wid_j": wj,
                "psi_i": psi_np[int(s_idx)].copy(),   # token-local psi
    ...
    # ------------------------------------------------------------
    print("-" * 80)
    print("TEMPLATE→TEMPLATE ATTENTION FLOWS (aggregated over sampled segments)  [EVENT-LOCAL]")
    print("-" * 80)
    pairs = []
    ...
    print("\n" + "-" * 80)
    print(f"TOP {topN} TOKEN→TOKEN LINKS (IDF-weighted; EVENT-LOCAL templates; COO k-sparse)")
    print("-" * 80)
    ...
    # ------------------------------------------------------------
    # Cluster context for top edges (token-local psi + global peers)
    # ------------------------------------------------------------
    print("\n" + "-" * 80)
    print(f"CLUSTER CONTEXT FOR TOP {topM} LINKS (token-local psi + template peers)")
    print("-" * 80)
    ...
    # ------------------------------------------------------------
    # Hierarchy parses per segment (densify only for first 64 tokens)
    # ------------------------------------------------------------
    print("\n" + "-" * 80)
    print("HIERARCHY (binary bracket parses) for sampled segments  [EVENT-LOCAL]")
    print("-" * 80)
    ...
    for seg_i, start in enumerate(starts, 1):
        ...
        key, nk = jax.random.split(key)
        src, dst, w_attn, *_ = segment_eventlocal_attention(seg_j, nk)

        src_np = np.array(src)
        dst_np = np.array(dst)
        w_np = np.array(w_attn)

        # build dense attention for parse window
