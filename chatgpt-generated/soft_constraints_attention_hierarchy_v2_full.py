...
# ============================================================
# Hierarchy / parse extraction (binary bracketing)
# ============================================================

def best_split_by_cross_mass(A: np.ndarray, i: int, j: int) -> Tuple[int, float]:
    """
    Choose split k in [i, j) minimizing cross attention mass between [i,k] and [k+1,j].
    """
    best_k = i
...
def build_parse(A: np.ndarray, tokens: List[str], i: int, j: int, depth: int = 0, max_depth: int = 6) -> str:
    """
    Recursive binary bracket parse; stops at max_depth or length 1.
    """
...
def main():
    ...
    print("=" * 80)
    print("HNET — ATTENTION + ANTI-HUB + SPARSE + HIERARCHY (SOFT TEMPLATES) v2 FULL")
    print("=" * 80)
    ...
    # ------------------------------------------------------------
    # Directional co-occurrence edges
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # N-gram hashed word embeddings (CPU -> JAX)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Cluster prototypes Zc, unary term, W_psi
    # ------------------------------------------------------------
    ...
    P_np = np.array(P)  # for reporting / peers

    # ------------------------------------------------------------
    # Cluster-to-cluster directional coupling J_pmi
    # ------------------------------------------------------------
    Pi = P[edges_i]
    Pj = P[edges_j]
    ...
    # IMPORTANT FIX: static Kk outside jit
    Kk = min(int(args.topk_attn), int(args.seg_len) - 1)
    NEG_LARGE = -1e9

    @jax.jit
    def segment_attention_sparse(word_ids_seg, noise_key):
        """
        Returns:
          A: (S,S) sparse attention
          base_logits, tpl_bias, dir_bias: (S,S) (dense; for diagnostics)
          pair_mass: (C,C)
        """
        Ew = Xw[word_ids_seg]
        psi = P[word_ids_seg]
        ...
        # mask far + self using NEG_LARGE (avoid -inf issues in top_k)
        ...
        def row_softmax(i):
            ...
            row = jnp.zeros((S,), dtype=logits.dtype).at[idxs].set(probs)
            return row

        A = jax.vmap(row_softmax)(jnp.arange(S))  # (S,S)

        pair_mass = psi.T @ A @ psi
        return A, base_logits, tpl_bias, dir_bias, pair_mass

    # ------------------------------------------------------------
    # Compile attention once
    # ------------------------------------------------------------
    starts = [random.randint(0, T - args.seg_len - 1) for _ in range(args.n_segs)]
    print("\nCompiling sparse segment attention (one-time JAX cost)...")
    dummy = tok_ids[starts[0]:starts[0] + args.seg_len].copy()
    dummy[dummy < 0] = 0
    _ = segment_attention_sparse(jnp.array(dummy, dtype=jnp.int32), key)[0].block_until_ready()
    print("Compilation finished.\n")

    # ------------------------------------------------------------
    # Precompute template peers: top words per template
    # ------------------------------------------------------------
    template_peers: Dict[int, List[int]] = {}
    for c in range(args.clusters):
        idx = np.argsort(-P_np[:, c])[:50]  # top 50 by membership
    ...
    # ------------------------------------------------------------
    # Run segments, collect edges + template flows
    # ------------------------------------------------------------
    all_edges = []
    ...
    for seg_i, start in enumerate(starts, 1):
        ...
        A, baseL, tplB, dirB, pair_mass = segment_attention_sparse(seg_j, nk)
        ...
        # extract nonzero edges
        for i in range(args.seg_len):
            row = A_np[i]
            ...
            for j in nz:
                ...
                if idf[wi] < args.min_idf_print or idf[wj] < args.min_idf_print:
                    continue
                if args.skip_punct_heavy and (is_punct_heavy(seg_words[i]) or is_punct_heavy(seg_words[j])):
                    continue
                ...
                all_edges.append({
                    ...
                    "w_j": seg_words[j],
                    "attn": float(row[j]),
                    "score_idf": float(row[j] * idf[wi] * idf[wj]),
    ...
    # ------------------------------------------------------------
    # Template→Template flow reporting
    # ------------------------------------------------------------
    print("-" * 80)
    ...
    print("\n" + "-" * 80)
    print(f"TOP {topN} TOKEN→TOKEN LINKS (IDF-weighted attention; sparse; anti-hub; filtered)")
    print("-" * 80)
    ...
    # ------------------------------------------------------------
    # Cluster context for Top 100 edges
    # ------------------------------------------------------------
    print("\n" + "-" * 80)
    ...
    for seg_i, start in enumerate(starts, 1):
        ...
        key, nk = jax.random.split(key)
        A, *_ = segment_attention_sparse(jnp.array(seg, dtype=jnp.int32), nk)
        A_np = np.array(A)

        L = min(64, args.seg_len)
        parse = build_parse(A_np[:L, :L], seg_words[:L], 0, L - 1, max_depth=6)
