...
def main():
    ...
    ap.add_argument("--report_per_template", type=int, default=30, help="how many edges to print per source template")
    ...
    print("=" * 80)
    print("HNET — EVENT-LOCAL TEMPLATES v3 (ROW-CENTERED BIASES + STRATIFIED REPORTING)")
    print("=" * 80)
    ...
    @jax.jit
    def segment_eventlocal_attention_v3(word_ids_seg, noise_key):
        """
        v3 adds:
          - row-centered tpl/dir bias before top-k
          - inferred J_pmi from provisional edges (as in v2)
        Returns:
          src, dst, w_attn (final)
          base_logits, tpl_bias_centered, dir_bias_centered
          psi
          pair_mass
        """
    ...
    print("\nCompiling event-local v3 COO attention (one-time JAX cost)...")
    dummy = tok_ids[starts[0]:starts[0] + args.seg_len].copy()
    dummy[dummy < 0] = 0
    _ = segment_eventlocal_attention_v3(jnp.array(dummy, dtype=jnp.int32), key)[0].block_until_ready()
    ...
    for seg_i, start in enumerate(starts, 1):
        ...
        src, dst, w_attn, baseL, tplB, dirB, psi, pair_mass = segment_eventlocal_attention_v3(seg_j, nk)
    ...
    # ------------------------------------------------------------
    print("-" * 80)
    print("TEMPLATE→TEMPLATE ATTENTION FLOWS (aggregated over sampled segments)  [EVENT-LOCAL v3]")
    print("-" * 80)
    pairs = []
    ...
    # histogram of src templates among topN
    ...
    print("\n" + "-" * 80)
    print(f"TOP {topN} TOKEN→TOKEN LINKS (IDF-weighted; EVENT-LOCAL v3; row-centered biases)")
    print("-" * 80)
    for r in all_edges[:topN]:
    ...
    # ------------------------------------------------------------
    # Stratified top edges per source template
    # ------------------------------------------------------------
    print("\n" + "-" * 80)
    ...
    # ------------------------------------------------------------
    print("\n" + "-" * 80)
    print("HIERARCHY (binary bracket parses) for sampled segments  [EVENT-LOCAL v3]")
    print("-" * 80)

    # Re-run per segment to get edges for parse, densify only for first 64 tokens
    for seg_i, start in enumerate(starts, 1):
        seg = tok_ids[start:start + args.seg_len].copy()
        seg[seg < 0] = 0
        seg_words = [vocab[int(w)] for w in seg]
        ...
        key, nk = jax.random.split(key)
        src, dst, w_attn, *_ = segment_eventlocal_attention_v3(seg_j, nk)
